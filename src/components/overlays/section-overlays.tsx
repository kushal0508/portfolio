...
function MagneticButton({ children, className, href, onClick, variant = "default" }: {
  ...
  const glowTargetRef = useRef({ x: 50, y: 50, on: false })
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    posRef.current = { x: cx * 0.28, y: cy * 0.28 }
    glowTargetRef.current = {
      x: ((e.clientX - rect.left) / rect.width) * 100,
      y: ((e.clientY - rect.top) / rect.height) * 100,
      on: true,
    }
    el.style.transform = `translate(${cx * 0.28}px, ${cy * 0.28}px)`
    if (innerRef.current) {
      innerRef.current.style.transform = `translate(${cx * 0.04}px, ${cy * 0.04}px)`
    }
    if (glowRef.current) {
      glowRef.current.style.opacity = '1'
      glowRef.current.style.background = `radial-gradient(circle at ${glowTargetRef.current.x}% ${glowTargetRef.current.y}%, rgba(168,158,254,0.35), transparent 55%)`
    }
  }, [])

  const onMouseLeave = useCallback(() => {
    posRef.current = { x: 0, y: 0 }
    glowTargetRef.current = { x: 50, y: 50, on: false }
...
function Card3D({ children, className, onClick, href, ...props }: {
  ...
  const applyStyles = useCallback(() => {
    ...
    el.style.transform = `perspective(1000px) rotateX(${r.x}deg) rotateY(${r.y}deg) translateZ(${lifted ? 16 : 0}px) translateY(${lifted ? -6 : 0}px) scale(${lifted ? 1.02 : 1})`
    if (borderGlowRef.current) {
      borderGlowRef.current.style.background = `radial-gradient(circle at ${m.x * 100}% ${m.y * 100}%, rgba(108,92,231,0.6), transparent 40%)`
    }
  }, [lifted])
  ...
  const onMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    rotateRef.current = { x: -y * 12, y: x * 12 }
    mousePosRef.current = {
  ...
  const onMouseLeave = useCallback(() => {
    ...
    const el = ref.current
    if (el) {
      el.style.transform = 'perspective(1000px) rotateX(0deg) rotateY(0deg) translateZ(0px) translateY(0px) scale(1)'
    }
  }, [])
  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background: "radial-gradient(circle at 50% 50%, rgba(108,92,231,0.6), transparent 40%)",
        }}
      />
      <div
        ref={glowRef}
...
export function Navigation() {
  ...
  // relative to the links container. Uses requestAnimationFrame for smooth updates.
  const measureIndicator = useCallback(() => {
    ...
    if (activeIndex < 0 || activeIndex >= buttons.length) {
      // Fallback: hide indicator if no valid active item
    ...
    // left = offset of the active element's left edge within the container
    // width = exact width of the active element text + padding
  ...
  // Update indicator whenever the active section changes or on resize
  useEffect(() => {
    measureIndicator()

    // Schedule a second measurement shortly after
  ...
  }, [activeNavItem, measureIndicator])
  ...
  return (
    <>
      <div className="scroll-progress" style={{ zIndex: 60 }} aria-hidden="true">
        <div
      ...
      >
        <div className="nav-inner">
          ...
          <div className="nav-actions flex items-center gap-3">
            ...
            <MagneticButton href={personalInfo.resume} variant="outline" className="hidden md:inline-flex nav-btn nav-btn-outline px-5 py-2 text-xs font-semibold uppercase tracking-wider rounded-full">
              Resume
            </MagneticButton>
            <button
...
export function SectionOverlays() {
  ...
  return (
    <div id="main-content" className="fixed inset-0 z-20 pointer-events-none" style={{ paddingTop: "calc(var(--nav-height) + 2rem)" }}>
      {/* Opening/Hero */}
      <div
        id="hero"
      ...
      >
        ...
        >
          <div
            className="hero-badge inline-flex items-center gap-2 px-4 sm:gap-2.5 sm:px-6 py-2 sm:py-2.5 rounded-full mb-8 sm:mb-10"
            style={{
          ...
          <h1 className="text-[clamp(2.2rem,8vw,6rem)] font-extrabold leading-[0.9] tracking-[-0.02em] mb-8">
            {[
              { text: personalInfo.shortName, gradient: true },
              { text: "Odoo Techno-Functional Intern", gradient: false },
              { text: "Frontend Developer", gradient: true },
            ].map((line, i) => (
              <span
                key={i}
          ...
          >
            Building scalable ERP solutions, responsive websites, and business-driven digital experiences.
          </p>

          <div
            className="hero-cta flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 pointer-events-auto"
            style={heroCtaStyle}
          >
            <MagneticButton
              href={personalInfo.resume}
              className="magnetic-btn magnetic-btn-primary px-9 py-3.5 rounded-xl text-sm font-semibold"
            ...
            >
              ...
              <span className="magnetic-glow" aria-hidden="true" style={{ background: "radial-gradient(circle at var(--glow-x, 50%) var(--glow-y, 50%), rgba(255,255,255,0.3), transparent 60%)" }} />
            </MagneticButton>
            <MagneticButton
              onClick={() => window.scrollTo({ top: 7500, behavior: "smooth" })}
              variant="outline"
              className="magnetic-btn magnetic-btn-outline px-9 py-3.5 rounded-xl text-sm font-semibold"
              aria-label="Get in Touch"
            >
              <span className="magnetic-content">Get in Touch</span>
      ...
      >
        <div className="max-w-4xl mx-auto w-full">
          ...
          >
            <Card3D className="text-center !p-6 interactive" href={`mailto:${personalInfo.email}`}>
              <div className="flex items-center justify-center w-12 h-12 mx-auto mb-3 rounded-xl" style={{ background: "rgba(108,92,231,0.1)", border: "1px solid rgba(108,92,231,0.2)" }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(108,92,231,0.8)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
                  <rect width="20" height="16" x="2" y="4" rx="2"></rect>
                  <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"></path>
                </svg>
              </div>
              <div className="text-xs font-semibold uppercase tracking-wider mb-1" style={{ color: "rgba(108,92,231,0.6)" }}>
                Email
              </div>
            ...
            <Card3D className="text-center !p-6 interactive" href={`tel:${personalInfo.phone.replace(/\s/g, "")}`}>
              <div className="flex items-center justify-center w-12 h-12 mx-auto mb-3 rounded-xl" style={{ background: "rgba(108,92,231,0.1)", border: "1px solid rgba(108,92,231,0.2)" }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(108,92,231,0.8)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            ...
            <Card3D className="text-center !p-6 interactive" href={`https://www.google.com/maps/place/${encodeURIComponent(personalInfo.location)}`}>
              <div className="flex items-center justify-center w-12 h-12 mx-auto mb-3 rounded-xl" style={{ background: "rgba(108,92,231,0.1)", border: "1px solid rgba(108,92,231,0.2)" }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(108,92,231,0.8)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            ...
            <Card3D className="text-center !p-6 interactive" href={personalInfo.resume}>
              <div className="flex items-center justify-center w-12 h-12 mx-auto mb-3 rounded-xl" style={{ background: "rgba(108,92,231,0.1)", border: "1px solid rgba(108,92,231,0.2)" }}>
                <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="rgba(108,92,231,0.8)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          ...
          <div
            className="flex flex-wrap items-center justify-center gap-3 sm:gap-4 mt-10 pointer-events-auto"
            style={{
              opacity: contactIntensity,
          ...
          >
            <MagneticButton href={personalInfo.github} variant="outline" className="magnetic-btn magnetic-btn-outline px-5 sm:px-7 py-3 rounded-xl text-sm font-semibold" aria-label="GitHub profile">
              <span className="magnetic-content">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
                GitHub
              </span>
            ...
            <MagneticButton href={personalInfo.linkedin} variant="outline" className="magnetic-btn magnetic-btn-outline px-5 sm:px-7 py-3 rounded-xl text-sm font-semibold" aria-label="LinkedIn profile">
              <span className="magnetic-content">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
                LinkedIn
              </span>
            ...
            <MagneticButton href={`mailto:${personalInfo.email}`} className="magnetic-btn magnetic-btn-primary px-5 sm:px-7 py-3 rounded-xl text-sm font-semibold">
              <span className="magnetic-content">Email Me</span>
              <span className="magnetic-glow" aria-hidden="true" style={{ background: "radial-gradient(circle at var(--glow-x, 50%) var(--glow-y, 50%), rgba(255,255,255,0.3), transparent 60%)" }} />
            </MagneticButton>
          </div>
