...
export function ChamberScene({ deviceTier = "high" }: { deviceTier?: DeviceTier }) {
  ...
  return (
    <group scale={intensity} position={[0, 0, -94]}>
      <ChamberHall sectionProgress={sectionProgress} deviceTier={deviceTier} />
      <AchievementCrystals sectionProgress={sectionProgress} deviceTier={deviceTier} />
      <ChamberAtmosphere sectionProgress={sectionProgress} deviceTier={deviceTier} />
      <ChamberLighting sectionProgress={sectionProgress} deviceTier={deviceTier} />
    </group>
  )
}

function ChamberHall({ sectionProgress, deviceTier }: { sectionProgress: number; deviceTier: DeviceTier }) {
  const isLow = deviceTier === "low"
  const columnGeometry = useMemo(() => new THREE.CylinderGeometry(0.6, 0.6, 14, isLow ? 6 : 12), [isLow])
  const archGeometry = useMemo(() => new THREE.TorusGeometry(4, 0.3, isLow ? 6 : 8, isLow ? 16 : 32, Math.PI), [isLow])
  const floorGeometry = useMemo(() => new THREE.CircleGeometry(15, isLow ? 32 : 64), [isLow])
...
function AchievementCrystals({ sectionProgress, deviceTier }: { sectionProgress: number; deviceTier: DeviceTier }) {
  const isLow = deviceTier === "low"
      ...
      {achievements.map((a, i) => (
        ...
        >
          <group position={a.pos} scale={sectionProgress}>
            ...
            {Array.from({ length: isLow ? 1 : 3 }).map((_, r) => (
              <mesh
                key={r}
                position={[0, 0, 0]}
                rotation={[Math.PI / 2, 0, (r / (isLow ? 1 : 3)) * Math.PI * 2]}
                scale={1 + r * 0.3}
              >
                <torusGeometry args={[1.5 + r * 0.5, 0.02, isLow ? 8 : 16, isLow ? 32 : 64]} />
                <meshBasicMaterial
                  color={a.color}
                  transparent
...
function ChamberAtmosphere({ sectionProgress, deviceTier }: { sectionProgress: number; deviceTier: DeviceTier }) {
  const isLow = deviceTier === "low"
  const count = isLow ? 600 : 1500
...
function ChamberLighting({ sectionProgress, deviceTier }: { sectionProgress: number; deviceTier: DeviceTier }) {
  const isLow = deviceTier === "low"
    ...
    <group>
      {lights.map((l, i) => (
        <group key={i} position={[Math.cos(l.angle) * l.radius, l.height, Math.sin(l.angle) * l.radius]}>
          ...
          >
            <sphereGeometry args={[0.2, isLow ? 8 : 16, isLow ? 8 : 16]} />
            <meshBasicMaterial
              color={l.color}
              transparent
              opacity={0.8}
